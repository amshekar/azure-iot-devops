    ...
    public partial class PatientMenu : Form
    {
        ...
        public void bgw_Completed(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            ...
            else
            {
                ...
                string connectionString = "Endpoint=sb://p-namespace.servicebus.windows.net/;SharedAccessKeyName=ListenPolicy;SharedAccessKey=W8RPcQyJQwbnPyfdTZxqVGRsoE5B8wRcW8E9uQfHY5U=";
                string eventHubName = "p-event";
                var eventHubClient = EventHubClient.CreateFromConnectionString(connectionString, eventHubName);
                IOTSimulator simulationObject = new IOTSimulator() {
                    SensorId = "IOTFACIOSENSOR01",
                    SimulationTime = DateTime.Now,
                    HR = Convert.ToDecimal(hr_current.ToString("N0")),
                 BP = Convert.ToDecimal(bp_current.ToString("N0")),
                  RR = Convert.ToDecimal(rr_current.ToString("N1")),
                 TM = Convert.ToDecimal(temp_current.ToString("N1")),
                   SPO = Convert.ToDecimal(spo2_current.ToString("N0")),
                  PatientName = txtBox_fName.Text,
                  PatientId = aPatient.patientId
                };

                var serializedString = JsonConvert.SerializeObject(simulationObject);
